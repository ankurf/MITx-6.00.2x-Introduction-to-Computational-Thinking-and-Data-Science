    ...
    def __init__(self, width, height):
        ...
        for w in range(0, width):
            for h in range(0, height):
                self.room[(w,h)] = RectangularRoom.dirty
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        #new random direction and new position, if the robot hits the wall
        while not self.room.isPositionInRoom(newPosition):
            self.direction = random.randint(1,359)
            newPosition = self.position.getNewPosition(self.direction, self.speed)
        #Robot is inside the room, set robot position and clean tile
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for run in range(num_trials):
        ...
        fracRoomCoverage = room.getNumCleanedTiles() / room.getNumTiles()
        count = 0
        while fracRoomCoverage <= float(min_coverage):
            for robo in robots:
                robo.updatePositionAndClean()
            fracRoomCoverage = float(room.getNumCleanedTiles()) / room.getNumTiles()
            count += 1
        ans.append(count)
